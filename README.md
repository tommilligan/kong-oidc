...
| `config.subject_verify_subdomain` || false | If `yes`, when injecting user, parse subject as a b64-urlencoded string, and verify that `subdomain` key matches subdomain in the `Host` header |
